...
def main():	   	  			  	 		  		  		    	 		 		   		 		  
    ...
    start_date = dt.datetime(2008,1,1)
    end_date = dt.datetime(2010,1,1)
    pv1, cum_ret, std_daily_ret, avg_daily_ret, trades1 = test_code_df(ManualStrategy(),symbol, start_date=start_date ,end_date=end_date,commission=9.95, impact=0.005)
    ...
    for day in trades1.index:
        ...
        if (holdings == -1000).all() and (trades1.loc[day] < 0).all():
            plt.vlines(day, pv1.loc[day]-0.2, pv1.loc[day], color="black")
    plt.legend()
    plt.savefig("strategies/manual_insample.png")
